...
class ArgosTranslator(BaseTranslator):
    ...
    def __init__(self, lang_in, lang_out, model, ignore_cache=False, **kwargs):
        ...
        try:
            from pdf2zh.argos_direct import get_argos_direct
            self.argos_direct = get_argos_direct()
        except ImportError:
            logger.warning(
                "argos_direct module not available, if you want to use argostranslate, please ensure ctranslate2 and sentencepiece are installed. If you don't use argostranslate translator, you can safely ignore this warning."
            )
            raise
    ...
    def translate(self, text: str, ignore_cache: bool = False):
        ...
        from pdf2zh.argos_direct import translate_text

        try:
            translated_text = translate_text(text, self.lang_in, self.lang_out)
            return translated_text
        except ValueError as e:
            if "No translation model found" in str(e):
                logger.error(
                    f"Translation model for {self.lang_in}->{self.lang_out} not found locally. "
                    "Please download it manually using: python download_models.py " +
                    f"{self.lang_out}"
                )
