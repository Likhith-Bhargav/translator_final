...
class TranslateConverter(PDFConverterEx):
    def __init__(
        ...
        if not envs:
            envs = {}
        # Prefer NLLB exclusively; default to NLLB if service unspecified
        available = [NLLBTranslator, M2M100Translator, ArgosTranslator]
        preferred = NLLBTranslator

        if not service_name:
            service_name = preferred.name
        for translator in available:
            if service_name == translator.name:
                ...
                break
        print(f"[DEBUG] Service requested: '{service}', Service name: '{service_name}', Available translators: {[t.name for t in available]}")
        if not self.translator:
            raise ValueError("Unsupported translation service: only 'nllb' is enabled")
    ...
    def receive_layout(self, ltpage: LTPage):
        ...
        def to_grapheme_clusters(text):
            """Splits text into Unicode grapheme clusters (visible characters, robust for Hindi etc.)"""
            return regex.findall(r'\X', text)

        ############################################################
        # A. 原文档解析
        y_line_threshold = 10  # Adjust as needed for your PDF scale
        prev_char = None
        for child in ltpage:
            if isinstance(child, LTChar):
                if is_hindi:
                    # For Hindi: Only start a new segment on big Y jump (new line/para), never on X drift
                    if not sstk:
                        sstk.append("")
                        pstk.append(Paragraph(child.y0, child.x0, child.x0, child.x0, child.y0, child.y1, child.size, False))
                    elif prev_char is not None and abs(child.y0 - prev_char.y0) > y_line_threshold:
                        sstk.append("")
                        pstk.append(Paragraph(child.y0, child.x0, child.x0, child.x0, child.y0, child.y1, child.size, False))
                    for cluster in to_grapheme_clusters(child.get_text()):
                        sstk[-1] += cluster
                    # Update paragraph/line bounding box info
                    ...
                    xt = child
                    xt_cls = 0
                    continue  # Skip generic math/placeholder code for Hindi
                cur_v = False
        ...
        # DEBUG: Show text segments before translation
        print(f"[DEBUG] Text segments to translate: {len(sstk)} segments")
        for i, segment in enumerate(sstk):
        ...
        def worker(s: str):  # 多线程翻译
            if not s.strip() or re.match(r"^\{v\d+\}$", s):  # 空白和公式不翻译
                return s
            try:
                # DEBUG: Add breakpoint or logging here
                print(f"[DEBUG] Translating: '{s}'")  # Temporary debug print
                new = self.translator.translate(s)
                print(f"[DEBUG] Translated to: '{new}'")  # Temporary debug print
                return new
        ...
        print(f"[DEBUG] Translation results: {len(news)} results")
        for i, (original, translated) in enumerate(zip(sstk, news)):
            print(f"[DEBUG] Result {i}: '{original}' -> '{translated}'")
        ...
        print(f"[DEBUG] Translator: {self.translator}, Source: {self.translator.lang_in}, Target: {self.translator.lang_out}")  # Debug translator info
        _x, _y = 0, 0
        ops_list = []
